...
|----|-----------|----------|--------|
|Interface {#IFNAME}({#IFALIAS}): Link down|<p>-</p>|<p>**Expression**: ({Mikrotik:net.if.status[ifOperStatus.{#SNMPINDEX}].last()}=2 and {Mikrotik:net.if.status[ifOperStatus.{#SNMPINDEX}].diff()}=1)</p><p>**Recovery expression**: </p>|average|
|Interface {#IFNAME}({#IFALIAS}): Link down (LLD)|<p>-</p>|<p>**Expression**: ({Mikrotik:net.if.status[ifOperStatus.{#SNMPINDEX}].last()}=2 and {Mikrotik:net.if.status[ifOperStatus.{#SNMPINDEX}].diff()}=1)</p><p>**Recovery expression**: </p>|average|
