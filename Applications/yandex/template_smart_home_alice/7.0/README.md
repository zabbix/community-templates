...
Шаблон предназначен для мониторинга устройств умного дома Яндекса (платформа «Дом с Алисой») через облачное API Яндекса по протоколу HTTP.

Сбор данных выполняется одним HTTP-запросом к методу `/v1.0/user/info` API умного дома. В ответе возвращается список всех устройств, комнат и домов, привязанных к аккаунту Яндекса, а также текущие показания их умений (capabilities) и датчиков (properties). На основе этих данных автоматическое обнаружение (LLD) создаёт элементы для умений и датчиков обнаруженных устройств.
...
|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Device capabilities with value mapping discovery||Dependent item|yandex.iot.capabilities.discovery.mapping[{#DEVICE.ID}]<p>**Preprocessing**</p><ul><li><p>JavaScript: `The text is too long. Please see the template.`</p></li><li><p>JSON Path: `$.capabilities`</p></li></ul>|
...
|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Device capabilities without value mapping discovery||Dependent item|yandex.iot.capabilities.discovery.not_mapping[{#DEVICE.ID}]<p>**Preprocessing**</p><ul><li><p>JavaScript: `The text is too long. Please see the template.`</p></li><li><p>JSON Path: `$.capabilities`</p></li></ul>|
...
|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Device sensors discovery||Dependent item|yandex.iot.sensors.discovery[{#DEVICE.ID}]<p>**Preprocessing**</p><ul><li><p>JavaScript: `The text is too long. Please see the template.`</p></li><li><p>JSON Path: `$.properties`</p></li></ul>|
