...
- **Snapshots missed by verification** — snapshots that **already existed when the last verify run started** and are *still* unverified. That run should have covered them and did not, so verification is silently skipping data: its scope does not reach them, or it is not completing. They are neither known-good nor known-bad. Raises a **WARNING** above `{$PBS.SNAPSHOT.UNVERIFIED.WARN}`.

  Backups taken *since* that run began are **not** counted — they are waiting their turn, not being skipped. The cutoff is the run's **start**, read from its `last-run-upid`, rather than its end: PBS lists a group's snapshots once, when it starts walking that group, so a backup that lands mid-run is genuinely not due yet. This distinction matters: on any datastore that backs up more often than it verifies (a daily backup with a weekly verify, say), simply counting *all* unverified snapshots would be non-zero roughly six days in seven and would tell you nothing at all.

  Neither is a snapshot whose own backup group holds an **older** snapshot verified *after* that snapshot was taken. PBS verifies a group **newest first** (`BackupInfo::sort_list(&mut list, false)`), from a list it takes once when it starts on the group — so if the run verified the older one, it had already passed the newer one. A missing verification state below that mark cannot mean "the run skipped it"; it can only mean something erased it afterwards.

  That something is normally a **pull sync job**. A pull overwrites the local manifest with the source's copy — so a snapshot verified an hour ago silently reverts to unverified, while its `backup-time` (the *source's*, not when the local copy was written) still predates the verify run. Snapshots repaired by a pull job's **`resync-corrupt`** are covered by the same rule, at any age, without the template having to identify them.

  How much a pull erases depends on the source. A manifest the **source** has verified carries the source's verification state across with it, so that snapshot is not unverified here at all — only snapshots the source has not yet reached lose their state to a sync. The flip side is that verification carried in from a source is indistinguishable from local verification in these counts: on a mirror, some snapshots read verified on the strength of the source having hashed its own copy. That has always been true of this item and is not introduced by the exemption; on a datastore whose integrity you want established *locally*, run a verify job here regardless of what the source does.

  The evidence has to survive, which gives the rule one blind spot worth knowing: a group that has **stopped taking backups** while a pull job still re-transfers it *from a source that does not verify*. With `ignore-verified` set on the verify job, its surviving older snapshots keep the timestamp of their *first* verification, which predates the re-synced ones — so nothing in the group proves the run reached them, and one snapshot per such group is counted from the first pull after a verify run until the next verify run clears it. A prune that removes a group's verified tail has the same effect. Both are visible as a count that rises on a pull and falls on a verify, never on a backup.

  That blind spot is also a **fallback**, and the two are the same mechanism seen from opposite ends. If a verify job stops running altogether, the cutoff freezes at its last run, so every later backup is newer than the cutoff and is never counted — the count does not rise on its own. What eventually raises it is precisely the case above: a re-synced snapshot losing the older sibling that exempted it, because prune removed the verified tail or the group is too short to have one. Late, indirect, and dependent on the retention policy, but it is the difference between late and never. Anyone tempted to suppress the noise should know they would be removing the fallback with it.

  What still counts, and must: a group with **no** verified older sibling at all — the wrong namespace, no verify job reaching it, a job that never completes — and a group whose verification stopped at some point in the past, since its verified snapshots predate the unverified ones rather than following them. The rule keys off evidence that a verify run demonstrably reached a snapshot, not off sync configuration, so it needs no `/admin/sync` call and no `RemoteAudit`.
...
While a verify job is **running**, the missed count is reported as nothing at all rather than as `0`: PBS reports no `last-run-endtime` for a run in progress — and forgets the previous one — so the cutoff is unknowable and a `0` would only mean "currently unknown". The item keeps its previous value until the run finishes; the failed count keeps updating throughout.

That suppression is unconditional — the value is withheld for the length of every verify run whatever it would have been, not only when it would have been wrong. Two consequences worth knowing: the missed count is not updated for the duration of a run (hours, on a large datastore), and a `nodata()` trigger added to this item would need a period longer than your longest verify run. The `last()` triggers shipped here are unaffected by design: with no new value, `last()` returns the value from before the run.
