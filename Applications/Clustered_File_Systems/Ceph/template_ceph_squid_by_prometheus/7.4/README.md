# Template App Ceph Squid by Prometheus endpoint

**Author:** Alexander Rydzewski ([GitHub](https://github.com/AlexRydzewski))  
**License:** MIT  
**Zabbix:** 7.4  
**Ceph:** 19.x (Squid), mgr `prometheus` module enabled
...
This complements (does not replace) the official [Zabbix Ceph by agent 2](https://www.zabbix.com/integrations/ceph) integration, which requires direct cluster access on the agent host.

## Related templates

| Template | Host |
|----------|------|
| This template (Squid) | One host per cluster — mgr Prometheus scrape |
| [Template App Ceph Tentacle by Prometheus endpoint](../template_ceph_tentacle_by_prometheus/7.4/) | Ceph 20.x only; use **one** Prometheus template per cluster |
...
Item keys use the `ceph19.*` prefix. See [Ceph mgr Prometheus](https://docs.ceph.com/en/latest/mgr/prometheus/) for metric semantics.
