...
|----|-----------|----------|--------|
|Interface:{#IFNAME}, Output Queue Length > 3|<p>Счетчик фиксирует длину очереди исходящих пакетов адаптера. Приемлемыми считаются значения 1 и 2. Но если этот показатель часто достигает уровня 3, 4 или более высоких отметок, это значит, что сетевой адаптер ввода/вывода не справляется с запросами сервера на передачу данных в сеть.</p>|<p>**Expression**: {OS Windows Server 2012-2016:perf_counter[" 510({#IFNAME}) 544",300].last()}>3</p><p>**Recovery expression**: </p>|warning|
|Interface:{#IFNAME}, Utilization Received > 50%|<p>Utilization Received = {ITEM.LASTVALUE}</p>|<p>**Expression**: {OS Windows Server 2012-2016:UtilizationReceived.[{#IFNAME}].last()}>50</p><p>**Recovery expression**: </p>|information|
|Interface:{#IFNAME}, Utilization Received > 90%|<p>Utilization Received = {ITEM.LASTVALUE}</p>|<p>**Expression**: {OS Windows Server 2012-2016:UtilizationReceived.[{#IFNAME}].last()}>90</p><p>**Recovery expression**: </p>|high|
|Interface:{#IFNAME}, Utilization Sent > 50%|<p>Utilization Sent = {ITEM.LASTVALUE}</p>|<p>**Expression**: {OS Windows Server 2012-2016:UtilizationSent.[{#IFNAME}].last()}>50</p><p>**Recovery expression**: </p>|information|
|Interface:{#IFNAME}, Utilization Sent > 90%|<p>Utilization Sent = {ITEM.LASTVALUE}</p>|<p>**Expression**: {OS Windows Server 2012-2016:UtilizationSent.[{#IFNAME}].last()}>90</p><p>**Recovery expression**: </p>|high|
|Состояние службы {#SERVICE.DISPLAYNAME} = {ITEM.LASTVALUE}|<p>Состояние службы {#SERVICE.DISPLAYNAME} = {ITEM.LASTVALUE} Если отслеживать состояние службы не нужно, просто отключите триггер</p>|<p>**Expression**: {OS Windows Server 2012-2016:service.info[{#SERVICE.NAME}].last()}<>0</p><p>**Recovery expression**: </p>|high|
|LogicalDisk {#FSNAME} Avg. Disk Queue Length > 2 (15 min)|<p>Среднее значение за 15 минут > 2 Показатель этого счетчика отражает среднее число ожидающих обработки запросов к диску на ввод и вывод данных. Если он стабильно выше 2, значит, в диске образовался «затор».</p>|<p>**Expression**: {OS Windows Server 2012-2016:perf_counter[" 236({#FSNAME}) 1400",60].avg(15m)}>2</p><p>**Recovery expression**: </p>|warning|
|LogicalDisk {#FSNAME} Avg. Disk Queue Length > 10 (3 min)|<p>Среднее значение за 3 минут > 10 Показатель этого счетчика отражает среднее число ожидающих обработки запросов к диску на ввод и вывод данных. Если он стабильно выше 10, значит, у системы серьезные проблема с производительностью диска.</p>|<p>**Expression**: {OS Windows Server 2012-2016:perf_counter[" 236({#FSNAME}) 1400",60].avg(3m)}>10</p><p>**Recovery expression**: </p>|high|
|LogicalDisk {#FSNAME} Free Space < 4%|<p>-</p>|<p>**Expression**: {OS Windows Server 2012-2016:perf_counter[" 236({#FSNAME}) 408",300].last()}<4</p><p>**Recovery expression**: </p>|high|
|LogicalDisk {#FSNAME} Free Space < 8%|<p>-</p>|<p>**Expression**: {OS Windows Server 2012-2016:perf_counter[" 236({#FSNAME}) 408",300].last()}<8</p><p>**Recovery expression**: </p>|average|
|Состояние службы {#SERVICE.DISPLAYNAME} = {ITEM.LASTVALUE} (LLD)|<p>Состояние службы {#SERVICE.DISPLAYNAME} = {ITEM.LASTVALUE} Если отслеживать состояние службы не нужно, просто отключите триггер</p>|<p>**Expression**: {OS Windows Server 2012-2016:service.info[{#SERVICE.NAME}].last()}<>0</p><p>**Recovery expression**: </p>|high|
...
|LogicalDisk {#FSNAME} Free Space < 4% (LLD)|<p>-</p>|<p>**Expression**: {OS Windows Server 2012-2016:perf_counter[" 236({#FSNAME}) 408",300].last()}<4</p><p>**Recovery expression**: </p>|high|
|LogicalDisk {#FSNAME} Free Space < 8% (LLD)|<p>-</p>|<p>**Expression**: {OS Windows Server 2012-2016:perf_counter[" 236({#FSNAME}) 408",300].last()}<8</p><p>**Recovery expression**: </p>|average|
...
|Interface:{#IFNAME}, Utilization Received > 50% (LLD)|<p>Utilization Received = {ITEM.LASTVALUE}</p>|<p>**Expression**: {OS Windows Server 2012-2016:UtilizationReceived.[{#IFNAME}].last()}>50</p><p>**Recovery expression**: </p>|information|
|Interface:{#IFNAME}, Utilization Received > 90% (LLD)|<p>Utilization Received = {ITEM.LASTVALUE}</p>|<p>**Expression**: {OS Windows Server 2012-2016:UtilizationReceived.[{#IFNAME}].last()}>90</p><p>**Recovery expression**: </p>|high|
|Interface:{#IFNAME}, Utilization Sent > 50% (LLD)|<p>Utilization Sent = {ITEM.LASTVALUE}</p>|<p>**Expression**: {OS Windows Server 2012-2016:UtilizationSent.[{#IFNAME}].last()}>50</p><p>**Recovery expression**: </p>|information|
|Interface:{#IFNAME}, Utilization Sent > 90% (LLD)|<p>Utilization Sent = {ITEM.LASTVALUE}</p>|<p>**Expression**: {OS Windows Server 2012-2016:UtilizationSent.[{#IFNAME}].last()}>90</p><p>**Recovery expression**: </p>|high|
