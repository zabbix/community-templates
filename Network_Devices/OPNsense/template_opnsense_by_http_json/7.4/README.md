...
- The API user needs read access to the following OPNsense API modules:
  ...
  - `diagnostics/interface/get_memory_statistics`, `get_netisr_statistics`,
...
| `{$OPNS.OPENVPN.ID.NOT_MATCHES}` | `^$` | OpenVPN instances or clients excluded from discovery. The default excludes nothing. |
| `{$OPNS.OPENVPN.CONTROL}` | `1` | Set to `0`, optionally per instance through macro context, to stop the OpenVPN down trigger. |
...
|------|-----|------|------|-------------|
| CPU: Utilization | `opns.cpu.util` | Calculated | % | Processor utilization, `100-last(//opns.cpu.idle)`. Zabbix has a multiplier but no offset, so this is the one CPU value a preprocessing step cannot produce. |
| CPU: Idle time | `opns.cpu.idle` | Dependent | % | Share spent idle. The one figure the process listing header states directly, which is why utilization is derived from it. |
...
| Firewall log: Blocked share | `opns.fwlog.block.pct` | Calculated | % | Share of blocked entries among the most recent logged packets, blocked divided by the window size. |
| Firewall log: Blocked entries | `opns.fwlog.block` | Dependent | – | Blocked entries in the window the endpoint returns. |
| Firewall log: Window size | `opns.fwlog.total` | Dependent | – | Number of entries the endpoint reports on. A fixed window of the most recent logged packets, so this normally sits at a constant value, and only rules with logging enabled are counted at all. |
...
| OpenVPN: sessions (raw) | `opns.openvpn.raw` | 1m | `/api/openvpn/service/search_sessions` |
...
discovery does not provide, inbound errors and link state, and leaves the existing rule
untouched. Both rules read the same master and discover the same interfaces, but no metric
exists twice, and no additional request is made.
...
| Name | Key | Description |
|------|-----|-------------|
| Interface {#IFALIAS} ({#IFNAME}): Link status | `opns.net.if.status[{#IFNAME}]` | Link up or down. Reads the link state the kernel reports for the interface, off the traffic master the template already polls. |
...
**Item Prototypes, phase 2:** ten values, split over two masters by where the firewall
already holds them.

Eight of them, bytes and packets in and out as totals and rates, read the phase 1 master.
`searchPhase1Action` aggregates exactly those four counters over every child SA of the
connection before it returns, `SessionsController.php` lines 82 to 99, and those child SAs are
the same ones `searchPhase2` lists. So the values are identical and cost no request. Note that
this makes the phase 2 byte items duplicates of the phase 1 byte items; they are kept so that
nobody loses an item on upgrade, but they could be dropped.

The remaining two, mode and state, describe a single child SA and exist nowhere else. They
come from `IPsec {#IPSECDESC}: phase 2 (raw)` (`opns.ipsec.phase2.raw[{#IPSECNAME}]`), an HTTP
agent prototype posting `id={#IPSECNAME}` to `/api/ipsec/sessions/search_phase2`, one request
per connection every five minutes, which is a rate that suits two values that rarely change.

---

### 14. OpenVPN Discovery

The VPN type the template did not cover yet, asked for in #706.

| Property | Value |
|----------|-------|
| Key | `opns.openvpn.discovery` |
| Type | Dependent (master: `opns.openvpn.raw`) |
| LLD Macros | `{#OVPN.ID}` -> `$.id`, `{#OVPN.DESC}` -> `$.description`, `{#OVPN.TYPE}` -> `$.type` |
| Filters | `{#OVPN.ID}` configurable via `{$OPNS.OPENVPN.ID.NOT_MATCHES}` |

One entity per instance, plus one per client connected to a server, whose identifier combines
the instance with the client address so the two never collide. Instances that are enabled but
not running are included as well, so a server that died stays visible instead of disappearing
from the host.

**Item Prototypes:** status, bytes received and sent as totals and as rates. All of them read
their record out of the response with plain JSONPath, there is no script in this branch.

**Trigger Prototypes:**

| Name | Severity | Description |
|------|----------|-------------|
| OpenVPN {#OVPN.DESC} ({#OVPN.TYPE}) is not up | **Average** | No working management socket for ten minutes. Set `{$OPNS.OPENVPN.CONTROL}` to `0`, per instance through macro context, for instances that are allowed to be down. |

Untested against a live installation: no OpenVPN instance was available. Endpoint, response
shape and field names are taken from `ServiceController.php` and `scripts/openvpn/ovpn_status.py`.
...
prototypes moved accordingly. Eight of them then turned out not to need that request at all:
phase 1 already aggregates bytes and packets over the same child SAs, so only mode and state
are still read per connection.
