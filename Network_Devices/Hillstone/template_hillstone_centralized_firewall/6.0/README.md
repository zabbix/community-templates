...
|----|-----------|----|----|
|CPU utilization|MIB: HILLSTONE-SYSTEM-MIB<p>CPU utilization in %.</p>|SNMP agent|hillstone.CPU.utilization<p>Update: 60</p>|
|CP Memory utilization|MIB: HILLSTONE-MODULE-MIB<p>Control Plan utilization.</p>|SNMP agent|hillstone.MemCP.utilization<p>Update: 60</p>|
|DP Memory utilization|MIB: HILLSTONE-MODULE-MIB<p>Data Plan utilization.</p>|SNMP agent|hillstone.MemDP.utilization<p>Update: 60</p>|
...
|----|-----------|----------|--------|---------|
|Hillstone: High CP memory utilization|The system is running out of free memory.|`min(/Hillstone by SNMP/hillstone.MemCP.utilization,5m)>{$MEMORY.UTIL.WARN}`|Warning| |
|Hillstone: High CPU utilization|The CPU utilization is too high. The system might be slow to respond.|`min(/Hillstone by SNMP/hillstone.CPU.utilization,5m)>{$CPU.UTIL.CRIT}`|Warning| |
|Hillstone: High DP memory utilization|The system is running out of free memory.|`min(/Hillstone by SNMP/hillstone.MemDP.utilization,5m)>{$MEMORY.UTIL.WARN}`|Warning| |
