# Принтеры Kyocera

## Description

ECOSYS M2540dn, ECOSYS M3540idn, ECOSYS M4132idn, ECOSYS M6230cidn

## Overview

ECOSYS M2540dn, ECOSYS M3540idn, ECOSYS M4132idn, ECOSYS M6230cidn may by and other...


Onli ru language.



## Author

Dmitrii

## Macros used

There are no macros links in this template.

## Template links

There are no template links in this template.

## Discovery rules

There are no discovery rules in this template.

## Items collected

|Name|Description|Type|Key and additional info|
|----|-----------|----|----|
|Название принтера|<p>-</p>|`SNMP agent`|Name<p>Update: 30s</p>|
|Real time toner  Yellow|<p>-</p>|`SNMP agent`|real-int-yellow<p>Update: 0;10s/7,08:00-18:00</p>|
|Max toner Black|<p>-</p>|`SNMP agent`|max-int-black<p>Update: 0;10s/7,08:00-18:00</p>|
|2 Жёлтый картрижд|<p>-</p>|`SNMP agent`|STRING3<p>Update: 0;10s/7,08:00-18:00</p>|
|Осталось Чёрного|<p>-</p>|`Calculated`|Black-Int<p>Update: 30s</p>|
|Осталось Жёлтого|<p>-</p>|`Calculated`|Yellow-Int<p>Update: 30s</p>|
|Reat time toner Magenta|<p>-</p>|`SNMP agent`|real-int-magenta<p>Update: 0;10s/7,08:00-18:00</p>|
|Осталось Пурпурного|<p>-</p>|`Calculated`|Magenta-Int<p>Update: 30s</p>|
|1 Черный Картридж|<p>-</p>|`SNMP agent`|STRING4<p>Update: 0;10s/7,08:00-18:00</p>|
|Max toner Yellow|<p>-</p>|`SNMP agent`|max-int-yellow<p>Update: 0;10s/7,08:00-18:00</p>|
|Осталось Голубого|<p>-</p>|`Calculated`|Cyan-Int<p>Update: 30s</p>|
|Real time toner Cyan|<p>-</p>|`SNMP agent`|real-int-cyan<p>Update: 0;10s/7,08:00-18:00</p>|
|Max toner Cyan|<p>-</p>|`SNMP agent`|max-int-cyan<p>Update: 0;10s/7,08:00-18:00</p>|
|Max toner Magenta|<p>-</p>|`SNMP agent`|max-int-magenta<p>Update: 0;10s/7,08:00-18:00</p>|
|Страниц всего напечатано|<p>-</p>|`SNMP agent`|StrInt<p>Update: 30s</p>|
|3 Голубой картридж|<p>-</p>|`SNMP agent`|STRING1<p>Update: 0;10s/7,08:00-18:00</p>|
|Расположен|<p>-</p>|`SNMP agent`|Location<p>Update: 30s</p>|
|Серийный номер|<p>-</p>|`SNMP agent`|SerialNumb<p>Update: 30s</p>|
|Real time toner Black|<p>-</p>|`SNMP agent`|real-int-black<p>Update: 0;10s/7,08:00-18:00</p>|
|4 Пурпурный картридж|<p>-</p>|`SNMP agent`|STRING2<p>Update: 30s</p>|


## Triggers

There are no triggers in this template.

# Принтре HP

## Description

ECOSYS M2540dn, ECOSYS M3540idn, ECOSYS M4132idn, ECOSYS M6230cidn

## Overview

ECOSYS M2540dn, ECOSYS M3540idn, ECOSYS M4132idn, ECOSYS M6230cidn may by and other...


Onli ru language.



## Author

Dmitrii

## Overview

ECOSYS M2540dn, ECOSYS M3540idn, ECOSYS M4132idn, ECOSYS M6230cidn may by and other...


Onli ru language.



## Author

Dmitrii

## Macros used

There are no macros links in this template.

## Template links

There are no template links in this template.

## Discovery rules

There are no discovery rules in this template.

## Items collected

|Name|Description|Type|Key and additional info|
|----|-----------|----|----|
|Страниц всего напечатано|<p>-</p>|`SNMP agent`|IntString<p>Update: 30s</p>|
|Осталось картриджа|<p>-</p>|`SNMP agent`|Int-Black<p>Update: 30s</p>|
|Real time toner|<p>-</p>|`SNMP agent`|real-int-black<p>Update: 30s</p>|
|Max toner|<p>-</p>|`SNMP agent`|max-int<p>Update: 30s</p>|
|Серийный номер|<p>-</p>|`SNMP agent`|SerialNumb<p>Update: 30s</p>|
|Название принтера|<p>-</p>|`SNMP agent`|Name<p>Update: 30s</p>|
|Наименование картириджа|<p>-</p>|`SNMP agent`|cartrig-name<p>Update: 30s</p>|


## Triggers

There are no triggers in this template.

